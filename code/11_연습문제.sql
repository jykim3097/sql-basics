...
SELECT CONCAT(FIRST_NAME, LAST_NAME) AS 이름, E.DEPARTMENT_ID
FROM EMPLOYEES E INNER JOIN DEPARTMENTS D ON E.DEPARTMENT_ID = D.DEPARTMENT_ID
...
SELECT CONCAT(FIRST_NAME, LAST_NAME) AS 이름, J.JOB_ID, JOB_TITLE
...
SELECT CONCAT(E1.FIRST_NAME, E1.LAST_NAME) AS 사원이름, CONCAT(E2.FIRST_NAME, E2.LAST_NAME) AS 매니저이름
FROM EMPLOYEES E1 LEFT OUTER JOIN EMPLOYEES E2 ON E1.MANAGER_ID = E2.EMPLOYEE_ID
ORDER BY 매니저이름;

--문제 11. 
----6. EMPLOYEES 테이블에서 left join하여 관리자(매니저)와, 매니저의 이름, 매니저의 급여까지 출력하세요
----매니저 아이디가 없는 사람은 배제하고 급여는 역순으로 출력하세요
SELECT * FROM EMPLOYEES;

SELECT DISTINCT E2.EMPLOYEE_ID AS 매니지아이디, CONCAT(E2.FIRST_NAME, E2.LAST_NAME) AS 매니저이름, E2.SALARY
FROM EMPLOYEES E1 LEFT OUTER JOIN EMPLOYEES E2 ON E1.MANAGER_ID = E2.EMPLOYEE_ID
