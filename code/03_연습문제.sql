...
select first_name || ' ' || last_name as 이름, salary+300 as 급여 from employees;
...
select first_name || ' ' || last_name || ' 사원의 급여는 ' || salary || '달러 입니다.' as 스티븐킹정보 from employees where first_name = 'Steven' and last_name = 'King';
...
select first_name || ' ' || last_name as 이름, job_id as 직무아이디 from employees where job_id like '%MAN' order by 직무아이디;